...
inteligente. Para isso são utilizados os dados de vídeo e profundidade providos
pelo sensor \textit{Kinect}~\cite{kinect_microsoft}. Por fim estas informações
são disponibilizadas para aplicações sensíveis ao contexto através do middleware
\textit{uOS}~\cite{fabriciobuzzeto2}. O projeto é composto por 4 módulos
(Figura~\ref{fig:relacao-modulos}) que serão detalhados a
seguir.
...
500 milissegundos, o Módulo de Rastreamento verifica se chegou algum resultado
de identificação. Caso tenha chegado, o Módulo de Rastreamento decide qual
identidade será atribuída ao respectivo usuário. Após o reconhecimento inicial,
com o objetivo de melhorar a confiança, o módulo de rastreamento realiza uma
nova tentativa de reconhecimento a cada 5 segundos. As taxas de 500
milissegundos para a verificação da chegada de resposta e de 5 segundos para
reverificar a identidade do usuário foram definidas arbitrariamente e se
...
àquele usuário. Desta forma a identidade é definida pela média ponderada das
...
pode ser realizado sem intrusão nas atividades do usuário.
Seu funcionamento consiste nos seguintes passos:

\begin{enumerate}
	\item Recebimento da imagem composta somente pelo usuário cujo
	 reconhecimento foi requisitado. 
	\item Pré-processamento da imagem, ou seja, conversão da imagem em escala de
	cinza.
	\item  Detecção facial na imagem. Caso nenhuma face seja encontrada interrompe o reconhecimento.
	...
	\item  Reconhecimento facial com a técnica \emph{Eigenfaces}. 
	\item  Envio da resposta contendo o nome do usuário com a face ``mais
	parecida'' e a confiança do reconhecimento.
\end{enumerate}

A detecção facial foi desenvolvida utilizando o método
\emph{Viola-Jones}~\cite{edsonma, violajones} cuja a implementação é encontrada na
...
suas imagens até o treinamento inicial para seu reconhecimento posterior. O
...
A última etapa do módulo de registro consiste no treinamento do sistema. Então,
uma lista de imagens é obtida com todas as imagens contidas no banco de faces.
Através desta lista, obtém-se a \textit{eigenface} média, os
\textit{eigenfaces} e os \textit{eigenvalues}. Para cada usuário cadastrado no
Sistema TRUE, suas imagens são projetadas no subespaço através do método PCA 
que reduz suas dimensionalidades, e são calculadas suas distâncias em relação
aos \emph{eigenfaces} obtendo uma lista de distâncias. Os \textit{eigenfaces},
os \textit{eigenvalues}, a \textit{eigenface} média e as listas de distâncias
são armazenados e podem ser utilizados pelo Módulo de Reconhecimento. Após o
treinamento, o Sistema TRUE é reiniciado para que o reconhecimento seja feito
utilizando as novas imagens e informações obtidas com o treinamento.

\subsection{Módulo de Integração}

O Módulo de Integração é responsável por disponibilizar ao \emph{smart space} os dados dos
usuários. Para isto, o Sistema TRUE disponibiliza um \textit{Driver} de Usuário
(\textit{UserDriver}) através do \emph{middleware} \textit{uOS}. Tal \textit{driver}
